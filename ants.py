...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min_range=0, max_range=float('inf')):
        ...
        moved = 0
        ref_place = self.place # so we don't change ant's place
        while (ref_place.bees == [] or moved < self.min_range) and ref_place.is_hive() == False:
            ref_place = ref_place.entrance
            moved += 1 
        if self.min_range <= moved <= self.max_range and ref_place.is_hive() == False:
            return bee_selector(ref_place.bees)
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chewing == 0:
            if self.place.bees != []:
                snack = bee_selector(self.place.bees)
                snack.reduce_health(snack.health)
...
# END Problem EC
    ...
    def __init__(self, health=1, which_queen=0):
        # BEGIN Problem EC
        super().__init__(health)
        QueenAnt.queen_counter += 1
        self.which_queen = QueenAnt.queen_counter
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        if self.which_queen != 1:
            if self.place.ant is self:
                self.place.ant = None
            super().reduce_health(self.health)
        else:
            super().action(gamestate)
            ref_place = self.place
            while ref_place.exit != None:
                ref_place = ref_place.exit
                if ref_place.ant != None:
                    ref_place.ant.buff()
                    if ref_place.ant.is_container() and ref_place.ant.contained_ant != None: 
                        ref_place.ant.contained_ant.buff()  
        # END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        self.health -= amount
        if self.health <= 0 and self.which_queen == 1:
            bees_win()
        # END Problem EC
    
    def remove_from(self, place): # "override Ant.remove_from in QueenAnt"
        if self.which_queen == 1:
            return None
